...
def completion_results(matches, cwd):
    ...
    for m in matches:
        ...
        try:
            ...
            with file_name.open("rt") as f:
                if not loc:  # Head of File
                    ...
                    # loc is a very-no-magic regex where only ^, $, / and \
                    # have meaaning
                    loc = loc[2:-2]  # strip /^   $/,
                    loc = loc.replace("\\/", "/")
                    loc = loc.replace("\\$", "$")
                    ...
                    for info in f:
                        ...
                        info = loc
                    # Add 4 lines if there are any left
                    info += "".join(itertools.islice(f, 0, 4))
                else:  # Line Number
                    line_no = int(loc)
        ...
        if not info:
            info = ""
        r = {"word": m["name"], "icase": 1, "menu": m["file"], "dup": 1, "info": info}
        res.append(r)
